...
library LibCommands {
  function buildInputs(bytes[] memory state, bytes4 selector, bytes32 indices) internal view returns (bytes memory ret) {
    ...
    for (uint256 i; i < 32; i++) {
      idx = uint8(indices[i]);
      if (idx == Constants.IDX_END_OF_ARGS) break;

      if (idx & Constants.IDX_VARIABLE_LENGTH != 0) {
        if (idx == Constants.IDX_USE_STATE) {
          ...
          uint256 arglen = state[idx & Constants.IDX_VALUE_MASK].length;
          require(arglen % 32 == 0, "Dynamic state variables must be a multiple of 32 bytes");
          count += arglen + 32;
        }
      } else {
        require(state[idx & Constants.IDX_VALUE_MASK].length == 32, "Static state variables must be 32 bytes");
        count += 32;
      }
      unchecked {
        free += 32;
    ...
    for (uint256 i; i < 32; i++) {
      ...
      if (idx & Constants.IDX_VARIABLE_LENGTH != 0) {
        if (idx == Constants.IDX_USE_STATE) {
          ...
          memcpy(state[idx & Constants.IDX_VALUE_MASK], 0, ret, free + 4, arglen);
          free += arglen;
        }
      } else {
        // Fixed length data; write it directly
        bytes memory statevar = state[idx & Constants.IDX_VALUE_MASK];
        assembly {
  ...
  function writeOutputs(bytes[] memory state, bytes1 index, bytes memory output) internal pure returns (bytes[] memory) {
    ...
    if (idx == Constants.IDX_END_OF_ARGS) return state;
    ...
    if (idx & Constants.IDX_VARIABLE_LENGTH != 0) {
      if (idx == Constants.IDX_USE_STATE) {
        ...
        uint256 IDX_VALUE_MASK = Constants.IDX_VALUE_MASK;
        assembly {
      ...
      state[idx & Constants.IDX_VALUE_MASK] = output;
    }
  ...
  function writeTuple(bytes[] memory state, bytes1 index, bytes memory output) internal view {
    ...
    if (idx == Constants.IDX_END_OF_ARGS) return;
