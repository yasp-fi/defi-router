...
abstract contract PeripheryPaymentsWithFee is PeripheryPayments {
    uint256 public constant LIMT_FEE_BIPS = 100;
    uint256 public constant MAX_FEE_BIPS = 10_000;

    function unwrapWETH9WithFee(uint256 amountMinimum, address recipient, uint256 feeBips, address feeRecipient)
        public
        payable
    {
        require(feeBips > 0 && feeBips <= MAX_FEE_BIPS);
        ...
        if (balanceWETH9 > 0) {
            ...
            uint256 feeAmount = balanceWETH9 * feeBips / MAX_FEE_BIPS;
            if (feeAmount > 0) {
    ...
    function sweepTokenWithFee(
        ...
        require(feeBips > 0 && feeBips <= LIMT_FEE_BIPS);

        uint256 balanceToken = IERC20(token).balanceOf(address(this));
        require(balanceToken >= amountMinimum, "Insufficient token");

        if (balanceToken > 0) {
            uint256 feeAmount = balanceToken * feeBips / MAX_FEE_BIPS;
            if (feeAmount > 0) {
